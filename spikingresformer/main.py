...
def parse_args():
    ...
    config_parser.add_argument(
        ...
        type=str,
        metavar="FILE",
        # The default config file to load if none is provided via the command line
        default="spikingresformer/configs/transfer/cifar10.yaml",
        help="YAML config file specifying default arguments",
    )

    parser = argparse.ArgumentParser(description='Training')

    # --- All arguments are defined here ONCE with their base defaults ---

    # Path options
    parser.add_argument('--data-path', default='split_data', # Your custom default path
                        help='path to dataset')
    parser.add_argument('--output-dir', default='output1', # Your custom default output
                        help='path to output directory')
    parser.add_argument('--resume', type=str,
                        help='resume from checkpoint')
    parser.add_argument('--transfer', default='spikingresformer_s.pth', # Your custom checkpoint
                        type=str, help='transfer from pretrained checkpoint')
    ...
    # --- The logic to parse arguments ---
    args_config, remaining = config_parser.parse_known_args()
    
    # If a config file is provided (either your default or from the command line)...
    if args_config.config:
        with open(args_config.config, 'r') as f:
            cfg = yaml.safe_load(f)
        # ...update the parser's defaults with the values from the YAML file.
        parser.set_defaults(**cfg)

    # Now, parse the final arguments. Any arguments passed on the command line
    # will override the values from the YAML file.
...
def load_data(
    ...
    elif dataset_type == 'My_Dataset':  # <-- The name you'll use in your YAML
        traindir = os.path.join(dataset_dir, 'train')
        valdir = os.path.join(dataset_dir, 'val')
        dataset_train = torchvision.datasets.ImageFolder(traindir)
        dataset_test = torchvision.datasets.ImageFolder(valdir)
        
        # --- Define augmentations and data loading ---
        # You can copy this part directly from the 'ImageNet' section
        # and adjust the mean/std if you have pre-calculated them for your dataset.
        # If not, the ImageNet stats are a reasonable starting point.
        augment_args = dict(
            scale=[0.08, 1.0],
            ratio=[3. / 4., 4. / 3.],
            hflip=0.5,
