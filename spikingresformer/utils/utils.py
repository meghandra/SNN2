...
def accuracy(output: torch.Tensor, target: torch.Tensor, topk=(1, )):
    ...
    """
    with torch.no_grad():
        # Determine the maximum k value possible based on the number of classes
        num_classes = output.size(1)
        max_k = max(topk)
        if max_k > num_classes:
            # If user asks for top-5 but there are only 2 classes,
            # only calculate up to top-2.
            max_k = num_classes
        ...
        for k in topk:
            if k <= max_k:
                ...
                # If we can't calculate a k, append 0.0 for that metric
